...
type Props = {
  ...
  onSelect: Dispatch<SetStateAction<GameType>> | ((gameType: GameType) => void);
};
...
export default function CreatePromotionNativeGameSelector({ selectedGameType, onSelect }: Props) {
  return (
      ...
      {availableGames.map((game) => {
        ...
        return (
          <button
            ...
            className={`w-full rounded-3xl border-2 p-5 text-left transition active:scale-[0.99] ${selected ? 'border-green-600 bg-green-50 shadow' : 'border-stone-100 bg-stone-50'}`}
          >
