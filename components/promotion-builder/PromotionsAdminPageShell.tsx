    ...
    <article className="rounded-3xl bg-white p-5 shadow-xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className={`mt-2 inline-block rounded-full px-3 py-1 text-xs font-black uppercase ${promotionBadgeClass(status)}`}>
            {status}
          </p>
        </div>
        <a
          href={`/admin/promotions/${promotion.id}/builder`}
          className="rounded-full bg-orange-50 px-4 py-2 text-center text-sm font-black text-[#FF6B00]"
        >
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
        {[
          ...
          <button
            ...
            className="rounded-2xl bg-stone-50 p-3 text-[#1F1F1F] transition hover:bg-orange-50 active:scale-[0.98]"
          >
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <button onClick={onCopy} className="rounded-2xl bg-[#1F1F1F] px-4 py-3 text-sm font-black text-white">
          {copied ? 'Copied!' : 'Copy Link'}
        </button>
        {status === 'active' && (
          <a href={playHref} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#FF6B00] px-4 py-3 text-center text-sm font-black text-white">
            Open Promotion
          </a>
        )}
        <a href={`/admin/promotions/${promotion.id}/print`} target="_blank" className="rounded-2xl bg-green-600 px-4 py-3 text-center text-sm font-black text-white">
          Print Kit
        </a>
        {status === 'draft' ? (
          <button onClick={onDelete} disabled={deleting} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">
            {deleting ? 'Deleting...' : 'Delete Draft'}
          </button>
        ) : status === 'active' || status === 'pending' ? (
          <button onClick={onEnd} disabled={ending} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">
            {ending ? 'Ending...' : 'End Promotion'}
          </button>
        ) : (
          <span className="rounded-2xl bg-stone-100 px-4 py-3 text-center text-sm font-black text-stone-500">
            Promotion Ended
          </span>
        )}
      </div>
      ...
      <section className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm font-bold text-stone-500">
              {mode === 'create' ? copy.create_tab_label : copy.manage_tab_label}
            </p>
          </div>
        ...
        <div className="mt-6 rounded-[2rem] bg-gradient-to-br from-[#FF6B00] to-[#E63939] p-6 text-white shadow-2xl shadow-orange-200">
          ...
          <h2 className="mt-3 text-4xl font-black leading-tight">
            {mode === 'create' ? copy.create_headline : copy.manage_headline}
          </h2>
          <p className="mt-3 text-sm font-semibold text-white/85">
            {mode === 'create' ? copy.create_subheadline : copy.manage_subheadline}
          </p>
        </div>

        <div className="mt-5 grid grid-cols-2 gap-3 rounded-3xl bg-white p-2 shadow-xl">
          <button onClick={() => setMode('create')} className={`rounded-2xl px-4 py-3 text-sm font-black ${mode === 'create' ? 'bg-green-600 text-white' : 'bg-white text-stone-500'}`}>
            {copy.create_tab_label}
          </button>
          <button onClick={() => setMode('manage')} className={`rounded-2xl px-4 py-3 text-sm font-black ${mode === 'manage' ? 'bg-[#1F1F1F] text-white' : 'bg-white text-stone-500'}`}>
            {copy.manage_tab_label}
          </button>
        </div>

        <section className="mt-5 rounded-3xl bg-white p-5 shadow-xl">
          <p className="text-sm font-black uppercase text-[#FF6B00]">
            {mode === 'create' ? copy.select_location_label : 'Restaurant Location'}
          </p>
          <select
          ...
          >
            ...
            {restaurants.map((restaurant) => (
              <option key={restaurant.id} value={restaurant.id}>{restaurantLocationLabel(restaurant)}</option>
            ))}
          </select>
          {selectedRestaurant && (
            <div className="mt-4 rounded-2xl bg-orange-50 p-4">
        ...
        <section className="mt-5 space-y-4">
          <div className="rounded-3xl bg-white p-5 shadow-xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-black uppercase text-[#FF6B00]">
                  {mode === 'create' ? 'Drafts in progress' : 'Managed promotions'}
                </p>
                <p className="mt-2 text-sm font-bold text-stone-500">
                  {mode === 'create'
                    ? 'Drafts appear here while they are still being built. Published campaigns move to Manage Promotions.'
              ...
              <button onClick={actions.loadPromotionMetrics} className="rounded-full bg-stone-100 px-4 py-3 text-xs font-black text-stone-700">
                Refresh Metrics
              </button>
            </div>

            {mode === 'manage' && (
              <div className="mt-4 grid grid-cols-5 gap-2 rounded-2xl bg-stone-50 p-2">
                {(['active', 'pending', 'draft', 'ended', 'all'] as Filter[]).map((item) => (
                  <button
                    key={item}
                    onClick={() => setFilter(item)}
                    className={`rounded-xl px-2 py-3 text-xs font-black ${filter === item ? 'bg-[#1F1F1F] text-white shadow' : 'bg-white text-stone-600'}`}
                  >
                    {filterLabel(item)}<br />{statusCounts[item]}
                  </button>
                ))}
              </div>
            )}
          ...
          {selectedRestaurant && visiblePromotions.length === 0 && (
            <div className="rounded-3xl bg-white p-6 shadow-xl">
              <p className="text-2xl font-black">{mode === 'create' ? copy.no_drafts_title : `No ${filter} promotions`}</p>
              <p className="mt-2 text-sm font-semibold text-stone-600">{mode === 'create' ? copy.no_drafts_copy : 'Switch filters to view other statuses.'}</p>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 px-3 py-4 sm:items-center">
          <section className="max-h-[88vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => setPerformance(null)} className="rounded-full bg-stone-100 px-4 py-3 text-sm font-black text-stone-700">
                Close
              </button>
            </div>
            ...
            <div className="mt-4 rounded-3xl bg-orange-50 p-4">
              <p className="text-sm font-black uppercase text-[#FF6B00]">Reward Breakdown</p>
              <div className="mt-3 space-y-2">
                {Object.entries(performance.rewardsBreakdown).length === 0 && (
                  <p className="text-sm font-bold text-stone-500">No coupons issued yet.</p>
                )}
                {Object.entries(performance.rewardsBreakdown).map(([reward, count]) => (
                  <div key={reward} className="flex items-center justify-between gap-3 rounded-2xl bg-white p-3">
                    <p className="text-sm font-black text-stone-700">{reward}</p>
                    <p className="rounded-full bg-[#FF6B00] px-3 py-1 text-sm font-black text-white">{count}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-4">
              <p className="text-sm font-black uppercase text-[#FF6B00]">Issued Coupon Ledger</p>
              <div className="mt-3 space-y-3">
                {performance.coupons.length === 0 && (
                  <div className="rounded-3xl bg-stone-50 p-5">
                    <p className="text-lg font-black">No issued coupons yet</p>
                    <p className="mt-1 text-sm font-bold text-stone-500">When customers play this promotion, issued coupons will appear here.</p>
                  </div>
                )}
                {performance.coupons.map((coupon) => (
                  <article key={coupon.id} className="rounded-3xl border border-stone-100 bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xl font-black">{coupon.coupon_code || 'No code'}</p>
                        <p className="mt-1 text-sm font-bold text-stone-500">{coupon.item_won} — {coupon.discount_type}</p>
                      </div>
