...
| **Restaurant Tool Library v1** | [`restaurant-tool-library-v1.md`](./restaurant-tool-library-v1.md) | **Canonical developer reference for `lib/restaurant-planner/tools/`.** Named, registered, individually-documented tools (`ToolDefinition`, `TOOL_REGISTRY`) the planner composes — distinct from `CAPABILITY_REGISTRY` in `tool-registry.ts`, a pre-existing naming collision explained in the doc. Behavior-preserving extraction, not new logic. |
| **Menu Editing Capability Boundary Audit v1** | [`menu-editing-capability-boundary-audit-v1.md`](./menu-editing-capability-boundary-audit-v1.md) | **Design audit — implemented 2026-07-12, see `ask-spinbite-ai-agent-v1.md`'s Capability Log for the shipped result.** Root-causes why pricing requests ("set Ras Malai to $7.99") get misrouted into the discount engine (`menu_pricing`'s `fixed_price` is structurally "lower than current," not a base-price setter), maps a full capability boundary matrix (Menu Editing / Menu Pricing / Promotions / Revenue Intelligence), and recommends building the already-stubbed `menu_agent` capability — recommendation B was approved and built. Flags an unresolved Constraint #1 ("no price manipulation") product decision, same class as the one `ask-spinbite-ai-agent-v1.md` already flagged for `special_*` — still unresolved, ships disabled pending that decision. |
| **menu_edit Pre-Merge Product Audit v1** | [`menu-edit-pre-merge-audit-v1.md`](./menu-edit-pre-merge-audit-v1.md) | **GO, 2026-07-12 (final pass).** Full production-launch-style review of the built `menu_edit` capability before merge — all 12 V1 operations traced end-to-end, 10 NL examples run for real through the parser/resolver. Found and then fixed both Important gaps before merge: (1) reused Decision Intelligence composers produced pricing-flavored copy on non-pricing proposals — closed via a new capability-aware composition layer, `lib/restaurant-planner/decision-intelligence.ts`; (2) `rename_item`/`update_description` had no guard against a bulk scope silently applying identical text to every matched item — closed via a resolve-time confirmation gate reusing the existing `TargetSelector` UI. Both fixes verified live, launch score 8/10. Three remaining Important findings (name uniqueness, silent partial-drop on bulk price edits, apply-without-a-proposal) are pre-existing and shared with `menu_pricing` — documented as platform-wide follow-up, not merge blockers. |
