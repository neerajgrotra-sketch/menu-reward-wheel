...
import { BarChart3, ChefHat, DoorOpen, Flame, Gift, QrCode, ShieldCheck, Sparkles, Star, Ticket, Trophy } from 'lucide-react';
...
function MysteryBoxShowcase({ playing }: { playing: boolean }) {
  return (
    <div className="rounded-[2rem] bg-gradient-to-br from-orange-50 to-amber-100 p-5 text-center shadow-inner ring-1 ring-orange-100">
      <p className="text-xs font-black uppercase tracking-[0.18em] text-[#FF6B00]">Mystery Box Reveal</p>
      <h3 className="mt-2 text-2xl font-black">Choose a box</h3>
      <div className="mt-5 grid grid-cols-3 gap-3">
        {[1, 2, 3].map((box) => (
          <div key={box} className={`relative flex h-28 items-center justify-center rounded-[1.5rem] bg-gradient-to-br from-[#FF6B00] to-[#E63939] shadow-xl ${playing && box === 2 ? 'scale-110 ring-4 ring-white' : ''}`}>
            <span className="absolute -top-2 text-xl">✨</span>
            <span className="text-5xl">{playing && box === 2 ? '🎉' : '🎁'}</span>
            <span className="absolute bottom-3 text-xs font-black uppercase tracking-wide text-white">Box {box}</span>
          </div>
        ))}
      </div>
      {playing && <div className="mt-5 rounded-2xl bg-white p-4 shadow-lg"><p className="text-xs font-black uppercase tracking-[0.14em] text-[#FF6B00]">You won</p><p className="mt-1 text-2xl font-black text-green-700">20% off your next meal</p></div>}
    </div>
  );
}
...
export default function LandingPageClient({ hero }: { hero: HomeHeroContent }) {
  ...
  const [mysteryPlaying, setMysteryPlaying] = useState(false);

  useEffect(() => {
    if (spinning || mysteryPlaying) return;
    const timer = window.setInterval(() => setActiveGame((game) => game === 'wheel' ? 'mystery' : 'wheel'), 5200);
    return () => window.clearInterval(timer);
  }, [spinning, mysteryPlaying]);

  function playDemo() {
    if (activeGame === 'mystery') {
      if (mysteryPlaying) return;
      setMysteryPlaying(true);
      setResult(null);
      setTimeout(() => {
        setResult('20% off your next meal');
        confetti({ particleCount: 170, spread: 110, origin: { y: 0.62 }, shapes: ['square', 'circle', 'star'] });
      }, 700);
      setTimeout(() => setMysteryPlaying(false), 3600);
      return;
    }
      ...
      <section id="top" className="relative px-4 pb-14 pt-8 text-center sm:px-6 md:pt-16">
        ...
        <div id="games" className="mt-10 flex flex-col items-center">
          <div className="mb-4 grid grid-cols-2 gap-2 rounded-full bg-white p-2 shadow-lg">
            <button onClick={() => { setActiveGame('wheel'); setResult(null); }} className={`rounded-full px-4 py-2 text-xs font-black uppercase ${activeGame === 'wheel' ? 'bg-[#1F1F1F] text-white' : 'text-stone-500'}`}>Spin Wheel</button>
            <button onClick={() => { setActiveGame('mystery'); setResult(null); }} className={`rounded-full px-4 py-2 text-xs font-black uppercase ${activeGame === 'mystery' ? 'bg-[#1F1F1F] text-white' : 'text-stone-500'}`}>Mystery Box</button>
          </div>
          <motion.div key={activeGame} initial={{ opacity: 0, x: activeGame === 'wheel' ? -18 : 18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }} className="w-full max-w-md rounded-[2rem] bg-white/80 p-4 shadow-2xl shadow-orange-200/60 ring-1 ring-orange-100">
            {activeGame === 'wheel' ? <RewardWheel rewards={landingRewards} rotation={rotation} spinning={spinning} /> : <MysteryBoxShowcase playing={mysteryPlaying || Boolean(result)} />}
          </motion.div>
          <button onClick={playDemo} disabled={spinning || mysteryPlaying} aria-label="Play demo game" className="mt-6 w-full max-w-xs rounded-full bg-gradient-to-r from-[#00C853] to-[#00A846] px-8 py-4 text-lg font-black text-white shadow-xl shadow-green-200 transition active:scale-95 disabled:bg-stone-400">{spinning ? 'Spinning...' : mysteryPlaying ? 'Opening...' : demoButtonLabel}</button>
          {result && <motion.p initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="mt-5 rounded-2xl bg-white px-6 py-3 text-2xl font-black shadow-lg">🎉 You won: {result}</motion.p>}
