...
function tableLabel(tp: TouchpointPrint) {
  return tp.section_name ? `${tp.section_name} — ${tp.name}` : tp.name;
}
...
function TableTentLayout({ r, tp, playUrl }: { r: RestaurantPrint; tp: TouchpointPrint; playUrl: string }) {
  ...
  return (
    <div className="mx-auto flex h-[5.5in] w-[3.5in] flex-col overflow-hidden rounded-[0.15in] shadow-2xl print:rounded-none print:shadow-none">
      <div className="flex flex-1 flex-col items-center justify-center bg-gradient-to-b from-[#FF6B00] to-[#FF8A00] px-[0.28in] pt-[0.28in] pb-[0.18in] text-center">
        ...
        {r.logo_url && <p className="mt-1 text-[0.11in] font-black uppercase tracking-[0.1em] text-white/80">{r.name}</p>}
        ...
        <p className="mt-2 text-[0.2in] font-black text-white/90">{tableLabel(tp)}</p>
        <p className="mt-1 text-[0.1in] font-bold text-white/75">No app required.</p>
      </div>
      <div className="flex shrink-0 flex-col items-center justify-center bg-white px-[0.22in] py-[0.2in] text-center">
        <div className="rounded-[0.18in] bg-[#1F1F1F] p-[0.14in] shadow-xl">
          <div className="rounded-[0.12in] bg-white p-[0.07in]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={qrUrl(playUrl, 280)} alt="Table QR code" className="h-[1.55in] w-[1.55in]" />
          </div>
          <p className="mt-1.5 break-all text-[0.07in] font-bold text-white/60">{r.slug}/{tp.touchpoint_code}</p>
        </div>
...
function TableStickerLayout({ r, tp, playUrl }: { r: RestaurantPrint; tp: TouchpointPrint; playUrl: string }) {
  ...
  return (
    <div className="mx-auto flex h-[3.5in] w-[3.5in] flex-col items-center justify-center overflow-hidden rounded-[0.18in] bg-white shadow-2xl print:rounded-none print:shadow-none" style={{ border: '0.06in solid #FF6B00' }}>
      <div className="flex flex-col items-center px-[0.18in] py-[0.15in] text-center">
        ...
        <p className="mb-1 text-[0.115in] font-black text-stone-700">{tableLabel(tp)}</p>
        <div className="mt-1 rounded-[0.14in] bg-[#1F1F1F] p-[0.11in]">
          <div className="rounded-[0.1in] bg-white p-[0.06in]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={qrUrl(playUrl, 220)} alt="QR code" className="h-[1.3in] w-[1.3in]" />
          </div>
        </div>
        <p className="mt-1.5 text-[0.09in] font-black text-stone-800">{r.name}</p>
        <p className="mt-0.5 text-[0.065in] font-bold text-stone-400">/r/{r.slug}/{tp.touchpoint_code}</p>
...
function CounterPosterLayout({ r, tp, playUrl }: { r: RestaurantPrint; tp: TouchpointPrint; playUrl: string }) {
  const tagline = qrTagline(r.experience_mode);
    ...
    <div className="mx-auto flex h-[8.5in] w-[5.5in] flex-col overflow-hidden rounded-[0.18in] bg-gradient-to-b from-[#FF6B00] via-[#FF8A00] to-[#FFF8F0] shadow-2xl print:rounded-none print:shadow-none">
      <div className="relative flex flex-1 flex-col px-[0.42in] py-[0.35in] text-center">
        ...
        <div className="relative z-10">
          <RestaurantBrand r={r} imgClass="mx-auto" />
          {r.logo_url && <p className="mt-2 text-[0.13in] font-black uppercase tracking-[0.1em] text-white/85">{r.name}</p>}
          ...
          <p className="mt-3 text-[0.22in] font-black text-white/90">{tableLabel(tp)}</p>
          <p className="mx-auto mt-2 max-w-[4.5in] text-[0.15in] font-bold leading-snug text-white/85">
            Scan with your phone&apos;s camera — no app needed.
          </p>
        </div>
        <div className="relative z-10 mx-auto mt-5 rounded-[0.28in] bg-white/95 p-[0.2in] shadow-2xl">
          <div className="rounded-[0.2in] bg-[#1F1F1F] p-[0.16in]">
            <div className="rounded-[0.14in] bg-white p-[0.09in]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={qrUrl(playUrl, 360)} alt="Table QR code" className="h-[2.25in] w-[2.25in]" />
            </div>
            <p className="mt-2 break-all text-center text-[0.08in] font-bold text-white/65">/r/{r.slug}/{tp.touchpoint_code}</p>
          </div>
        </div>
...
function WindowDecalLayout({ r, tp, playUrl }: { r: RestaurantPrint; tp: TouchpointPrint; playUrl: string }) {
  const tagline = qrTagline(r.experience_mode);
  return (
    <div className="mx-auto flex h-[6in] w-[4in] flex-col overflow-hidden rounded-[0.18in] bg-white shadow-2xl print:rounded-none print:shadow-none">
      <div className="bg-[#FF6B00] px-[0.25in] py-[0.2in] text-center">
        <RestaurantBrand r={r} imgClass="mx-auto" />
        {r.logo_url && <p className="mt-1 text-[0.12in] font-black uppercase tracking-wide text-white/85">{r.name}</p>}
      </div>
      <div className="flex flex-1 flex-col items-center justify-center px-[0.28in] py-[0.22in] text-center">
        ...
        <p className="mt-1 text-[0.18in] font-black text-[#FF6B00]">{tableLabel(tp)}</p>
        <p className="mt-2 text-[0.12in] font-semibold text-stone-500">Scan with your camera — no app needed.</p>
        <div className="mt-4 rounded-[0.22in] bg-[#1F1F1F] p-[0.17in] shadow-xl">
          <div className="rounded-[0.16in] bg-white p-[0.09in]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={qrUrl(playUrl, 280)} alt="QR code" className="h-[1.85in] w-[1.85in]" />
          </div>
          <p className="mt-1.5 break-all text-center text-[0.075in] font-bold text-white/60">/r/{r.slug}/{tp.touchpoint_code}</p>
        </div>
      </div>
...
function TakeoutInsertLayout({ r, tp, playUrl }: { r: RestaurantPrint; tp: TouchpointPrint; playUrl: string }) {
  const tagline = qrTagline(r.experience_mode);
  return (
    <div className="mx-auto flex h-[2.75in] w-[4.25in] overflow-hidden rounded-[0.12in] bg-white shadow-2xl print:rounded-none print:shadow-none" style={{ border: '0.04in solid #FF6B00' }}>
      <div className="flex shrink-0 items-center justify-center bg-[#1F1F1F] px-[0.14in] py-[0.14in]">
        <div>
          <div className="rounded-[0.1in] bg-white p-[0.06in]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={qrUrl(playUrl, 190)} alt="QR code" className="h-[1.3in] w-[1.3in]" />
          </div>
          <p className="mt-0.5 break-all text-center text-[0.065in] font-bold text-white/50">{tp.touchpoint_code}</p>
        </div>
      </div>
      <div className="flex flex-1 flex-col justify-center px-[0.18in] py-[0.14in]">
        ...
        <h3 className="mt-1 text-[0.18in] font-black leading-tight text-[#FF6B00]">{tagline}</h3>
        <p className="mt-0.5 text-[0.11in] font-black text-stone-700">{tableLabel(tp)}</p>
...
function SocialGraphicLayout({ r, tp, playUrl }: { r: RestaurantPrint; tp: TouchpointPrint; playUrl: string }) {
  const tagline = qrTagline(r.experience_mode);
  const subCopy = r.experience_mode === 'menu_only'
    ? 'Scan to view our full menu — no app needed'
    : 'Scan to order & win a reward — no app needed';
  return (
    <div
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center px-[0.44in] text-center">
        ...
        <h2 className="mt-2 text-[0.56in] font-black leading-[0.92] text-white drop-shadow-lg">{tagline}</h2>
        <p className="mt-1 text-[0.16in] font-black text-white/90">{tableLabel(tp)}</p>
        <div className="mt-4 rounded-[0.22in] bg-white/95 p-[0.14in] shadow-2xl">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={qrUrl(playUrl, 280)} alt="QR code" className="h-[1.7in] w-[1.7in] rounded-[0.1in]" />
        </div>
        <p className="mt-2 text-[0.11in] font-bold text-white/88">{subCopy}</p>
        <div className="mt-2 flex items-center gap-1.5 text-[0.1in] font-black text-white/70">
          <SpinBiteMark className="h-[0.17in] w-[0.17in] text-[0.09in]" />
          <span>SpinBite</span>
        </div>
      </div>
...
function TouchpointPrintContent() {
  const params       = useParams<{ restaurantId: string; touchpointId: string }>();
  ...
  const [touchpoint, setTouchpoint] = useState<TouchpointPrint | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      // restaurant_touchpoints not in generated types — use untyped client
      type AnyClient = { from: (t: string) => { select: (c: string) => { eq: (k: string, v: string) => { eq: (k: string, v: string) => { maybeSingle: () => Promise<{ data: TouchpointPrint | null; error: { message: string } | null }> } } } } };
      const { data: tData, error: tErr } = await (supabase as unknown as AnyClient)
      ...
      if (tErr || !tData) { setError('Touchpoint not found.'); setLoading(false); return; }

      const tp = tData as TouchpointPrint;
      setRestaurant(rData as RestaurantPrint);
      setTouchpoint(tp);
      setPlayUrl(`${window.location.origin}/r/${rData.slug}/${tp.touchpoint_code}`);
      setLoading(false);
    }
    load();
  }, [params.restaurantId, params.touchpointId, supabase]);

  if (loading) return <main className="min-h-screen bg-[#FFF8F0] p-6 font-bold text-stone-600">Loading print kit…</main>;
  if (error || !restaurant || !touchpoint) return <main className="min-h-screen bg-[#FFF8F0] p-6 font-bold text-red-700">{error || 'Print kit unavailable.'}</main>;
  ...
  return (
    <main className="min-h-screen bg-stone-200 px-4 py-6 text-[#1F1F1F] print:bg-white print:p-0">
        ...
        <div className="flex items-center justify-between gap-3 rounded-3xl bg-white p-4 shadow-xl">
          <div>
            <p className="text-sm font-black uppercase text-[#FF6B00]">
              Table QR Kit — {touchpoint.section_name ? `${touchpoint.section_name}: ` : ''}{touchpoint.name}
            </p>
            <p className="text-xs font-bold text-stone-500">
              Session-linked QR — customers scan to see the menu and track their table orders.
            </p>
          </div>
      ...
      <div className="print:m-0 print:flex print:h-screen print:items-center print:justify-center">
        {format === 'table-tent'     && <TableTentLayout     r={restaurant} tp={touchpoint} playUrl={playUrl} />}
        {format === 'table-sticker'  && <TableStickerLayout  r={restaurant} tp={touchpoint} playUrl={playUrl} />}
        {format === 'counter-poster' && <CounterPosterLayout r={restaurant} tp={touchpoint} playUrl={playUrl} />}
        {format === 'window-decal'   && <WindowDecalLayout   r={restaurant} tp={touchpoint} playUrl={playUrl} />}
        {format === 'takeout-insert' && <TakeoutInsertLayout r={restaurant} tp={touchpoint} playUrl={playUrl} />}
        {format === 'social-graphic' && <SocialGraphicLayout r={restaurant} tp={touchpoint} playUrl={playUrl} />}
      </div>
    </main>
  );
}
