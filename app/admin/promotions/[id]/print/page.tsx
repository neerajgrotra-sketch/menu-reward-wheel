...
export default function PromotionPrintKitPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setPromotion(currentPromotion); setRestaurant(currentRestaurant); setPlayUrl(`${window.location.origin}/play/${currentRestaurant.slug}/${currentPromotion.slug}`); setLoading(false);
    }
    load();
  }, [params.id, supabase]);
  ...
  return <main className="min-h-screen bg-stone-200 px-4 py-6 text-[#1F1F1F] print:bg-white print:p-0">
    ...
    <div className="no-print mx-auto mb-5 flex max-w-[8.5in] items-center justify-between gap-3 rounded-3xl bg-white p-4 shadow-xl"><div><p className="text-sm font-black uppercase text-[#FF6B00]">Print Kit Preview</p><p className="text-xs font-bold text-stone-500">Designed to fit on one letter-size page. Use Print / Save as PDF.</p></div><div className="flex gap-2"><a href="/admin/promotions?mode=manage" className="rounded-full bg-stone-100 px-4 py-3 text-sm font-black text-stone-700">Back</a><button onClick={() => window.print()} className="rounded-full bg-green-600 px-5 py-3 text-sm font-black text-white shadow-lg">Print / Save PDF</button></div></div>
    <section className="mx-auto flex h-[10.15in] w-full max-w-[7.75in] flex-col overflow-hidden rounded-[0.18in] bg-[#FFF8F0] shadow-2xl print:h-[10.15in] print:w-[7.75in] print:max-w-none print:rounded-none print:shadow-none">
      <div className="relative flex h-full flex-col bg-gradient-to-b from-[#FF6B00] via-[#FF8A00] to-[#FFF8F0] px-[0.42in] py-[0.32in] text-center">
        ...
        <div className="relative z-10"><p className="text-[0.25in] font-black uppercase tracking-[0.18em] text-white/90">{restaurant.name}</p>{restaurantAddress && <p className="mt-1 text-[0.15in] font-bold text-white/85">{restaurantAddress}</p>}<h1 className="mt-5 text-[0.72in] font-black leading-[0.88] tracking-tight text-white drop-shadow-md">Scan to<br />Spin & Win</h1><p className="mx-auto mt-3 max-w-[6.3in] text-[0.18in] font-black leading-tight text-white">Unlock a surprise reward before you order. No app download required.</p></div>
        <div className="relative z-10 mt-4 rounded-[0.35in] bg-white/95 p-[0.23in] shadow-2xl"><div className="grid grid-cols-[2.2in_1fr_1.95in] items-center gap-[0.2in]">
          <div className="rounded-[0.28in] bg-orange-50 px-2 py-3"><p className="mb-1 text-[0.11in] font-black uppercase tracking-[0.12em] text-[#E63939]">Spin & Win</p><WheelGraphic /><p className="mt-1 text-[0.115in] font-black text-[#FF6B00]">Play for discounts, free items, and featured rewards.</p></div>
          <div className="text-left"><p className="text-[0.105in] font-black uppercase tracking-[0.14em] text-[#E63939]">Featured Promotion</p><h2 className="mt-1 text-[0.27in] font-black leading-tight text-[#1F1F1F]">{promotion.name}</h2><p className="mt-2 text-[0.135in] font-bold leading-snug text-stone-600">Scan the code, spin the wheel, and reveal your reward instantly on your phone.</p></div>
        ...
        <div className="relative z-10 mt-4 rounded-[0.32in] bg-[#1F1F1F] p-[0.28in] text-left text-white shadow-2xl"><p className="text-[0.24in] font-black leading-tight">How it works</p><div className="mt-3 grid grid-cols-4 gap-3 text-center"><div className="rounded-[0.16in] bg-white/10 p-3"><p className="text-[0.18in] font-black text-[#FFD166]">1</p><p className="mt-1 text-[0.105in] font-black leading-tight">Scan the QR code</p></div><div className="rounded-[0.16in] bg-white/10 p-3"><p className="text-[0.18in] font-black text-[#FFD166]">2</p><p className="mt-1 text-[0.105in] font-black leading-tight">Spin the reward wheel</p></div><div className="rounded-[0.16in] bg-white/10 p-3"><p className="text-[0.18in] font-black text-[#FFD166]">3</p><p className="mt-1 text-[0.105in] font-black leading-tight">Show coupon to staff</p></div><div className="rounded-[0.16in] bg-white/10 p-3"><p className="text-[0.18in] font-black text-[#FFD166]">4</p><p className="mt-1 text-[0.105in] font-black leading-tight">Redeem within {couponMinutes} minutes</p></div></div><p className="mt-3 text-center text-[0.115in] font-bold text-white/70">Each coupon is valid for {couponMinutes} minutes after it is issued.</p>{expiry && <p className="mt-1 text-center text-[0.105in] font-bold text-white/50">Promotion ends: {expiry}</p>}</div>
