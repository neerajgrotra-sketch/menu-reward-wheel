...
export default function PromotionPrintKitPage() {
  ...
  return <main className="min-h-screen bg-stone-200 px-4 py-6 text-[#1F1F1F] print:bg-white print:p-0">
    ...
    <div className="no-print mx-auto mb-5 flex max-w-[8.5in] items-center justify-between gap-3 rounded-3xl bg-white p-4 shadow-xl"><div><p className="text-sm font-black uppercase text-[#FF6B00]">Print Kit Preview</p><p className="text-xs font-bold text-stone-500">Designed to fit on one letter-size page. QR code is permanent for this restaurant location.</p></div><div className="flex gap-2"><a href="/admin/promotions?mode=manage" className="rounded-full bg-stone-100 px-4 py-3 text-sm font-black text-stone-700">Back</a><button onClick={() => window.print()} className="rounded-full bg-green-600 px-5 py-3 text-sm font-black text-white shadow-lg">Print / Save PDF</button></div></div>
