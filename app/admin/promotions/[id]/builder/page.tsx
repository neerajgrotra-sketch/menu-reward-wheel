...
export default function PromotionBuilderPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const menuResult = await supabase
        ...
        .order('name');
      const loadedMenus = menuResult.data || [];
  ...
  const errors = useMemo(() => {
    ...
    rewards.forEach((reward, index) => {
      ...
      if (
        ...
        (!reward.reward_value || reward.reward_value < 1 || reward.reward_value > 100)
      ) {
        messages.push(`Reward ${index + 1} needs a valid discount.`);
      }
      if (!reward.daily_limit || reward.daily_limit < 1) {
        messages.push(`Reward ${index + 1} needs a daily limit.`);
      }
    });
  ...
  async function saveDraft() {
    ...
    const promotionUpdate = await supabase
      .from('promotions')
      .update({
    ...
    if (rewards.length) {
      const insertRewards = await supabase.from('promotion_rewards').insert(
        rewards.map((reward) => ({
          promotion_id: promotion.id,
          restaurant_id: restaurant.id,
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#FFF8F0] px-3 py-6 text-[#1F1F1F] sm:px-6">
      <section className="mx-auto max-w-6xl space-y-6">
        <div className="flex items-center justify-between gap-4">
          <a href={`/admin/promotions?slug=${restaurant.slug}`} className="rounded-full bg-white px-5 py-3 text-sm font-black text-[#FF6B00] shadow">Back</a>
          ...
          <div className="grid min-w-0 gap-6 lg:grid-cols-[1fr_.82fr] lg:items-center">
            <div className="min-w-0">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="shrink-0 rounded-full bg-white px-3 py-2 text-xs font-black capitalize text-[#1F1F1F] shadow">{promotion.status}</span>
              </div>
              ...
              <div className="mt-5 rounded-3xl bg-white/15 p-4 backdrop-blur">
                ...
                <div className="mt-3 rounded-2xl bg-white/15 p-3">
                  <p className="break-all text-xs font-black text-white/85">{full}</p>
                  <button onClick={copyLink} className="mt-2 w-full rounded-full bg-white px-4 py-2 text-xs font-black text-[#FF6B00]">{copied ? 'Copied!' : 'Copy Promotion Link'}</button>
                </div>
              </div>
            </div>
            ...
            <div className="min-w-0 rounded-[2rem] bg-white/90 p-3 text-[#1F1F1F] shadow-2xl ring-1 ring-white/50 sm:p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.14em] text-stone-400">Live wheel</p>
                  {result && <p className="text-sm font-black text-green-700">🎉 {result}</p>}
                </div>
                <button onClick={testSpin} disabled={!rewards.length || spinning} className="rounded-full bg-[#1F1F1F] px-5 py-2 text-sm font-black text-white shadow-lg disabled:bg-stone-300">{spinning ? 'Spinning...' : 'Test'}</button>
              </div>
        ...
        <div className="rounded-[2rem] bg-white p-5 shadow-xl">
          ...
          <select value={menuId} onChange={(event) => setMenuId(event.target.value)} className="mt-3 w-full rounded-2xl border border-stone-200 px-4 py-4 font-black outline-none focus:border-[#FF6B00]">
        ...
        <div className="rounded-[2rem] bg-white p-5 shadow-xl">
          <div className="flex items-start justify-between gap-4">
            ...
            <button onClick={addCustom} disabled={rewards.length >= MAX} className="rounded-2xl bg-[#1F1F1F] px-4 py-3 text-sm font-black text-white disabled:bg-stone-300">Custom</button>
          </div>
            ...
            {menuItems.map((item) => {
              ...
              return (
                <div key={item.id} className="flex items-center justify-between gap-4 rounded-2xl border border-stone-100 p-4">
                  <div className="min-w-0"><p className="truncate text-lg font-black">{item.name}</p><p className="text-sm font-bold text-stone-500">{money(item.price)}</p></div>
                  <button onClick={() => addItem(item)} disabled={added || rewards.length >= MAX} className="rounded-full bg-[#FF6B00] px-4 py-2 text-sm font-black text-white disabled:bg-stone-300">{added ? 'Added' : 'Add'}</button>
                </div>
              );
            })}
            {!menuItems.length && <p className="rounded-2xl bg-stone-50 p-4 text-sm font-bold text-stone-500 md:col-span-2">No items found in this menu.</p>}
          </div>
        </div>
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            {rewards.map((reward, index) => (
              <div key={reward.temp_id} className="rounded-3xl bg-stone-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div><p className="text-xs font-black uppercase text-stone-400">Segment {index + 1}</p><p className="text-xl font-black">{reward.label}</p></div>
                ...
                {!reward.menu_item_id && <input value={reward.label} onChange={(event) => updateReward(reward.temp_id, { label: event.target.value, custom_name: event.target.value })} className="mt-4 w-full rounded-2xl border border-stone-200 px-4 py-3 font-bold" />}
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <label className="text-sm font-black text-stone-700">Reward Type<select value={reward.reward_type} onChange={(event) => updateReward(reward.temp_id, { reward_type: event.target.value as RewardType })} className="mt-1 w-full rounded-2xl border border-stone-200 px-3 py-3 font-bold"><option value="free">Free item</option><option value="discount">% Discount</option><option value="custom">Custom</option></select></label>
                  {reward.reward_type === 'discount' && <label className="text-sm font-black text-stone-700">Discount %<input type="number" min={1} max={100} value={reward.reward_value || ''} onChange={(event) => updateReward(reward.temp_id, { reward_value: Number(event.target.value) })} className="mt-1 w-full rounded-2xl border border-stone-200 px-3 py-3 font-bold" /></label>}
                  <label className="text-sm font-black text-stone-700">Daily Limit<input type="number" min={1} value={reward.daily_limit} onChange={(event) => updateReward(reward.temp_id, { daily_limit: Number(event.target.value) })} className="mt-1 w-full rounded-2xl border border-stone-200 px-3 py-3 font-bold" /></label>
                  <label className="text-sm font-black text-stone-700">Weight<select value={reward.weight_label} onChange={(event) => updateReward(reward.temp_id, { weight_label: event.target.value as WeightLabel })} className="mt-1 w-full rounded-2xl border border-stone-200 px-3 py-3 font-bold"><option value="Common">Common</option><option value="Normal">Normal</option><option value="Rare">Rare</option></select></label>
                </div>
              </div>
            ))}
            {!rewards.length && <p className="rounded-2xl bg-stone-50 p-4 text-sm font-bold text-stone-500 lg:col-span-2">Add rewards from the selected menu to configure the wheel.</p>}
          </div>
        </div>
