...
export default function PromotionBuilderPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const rw = await supabase.from('promotion_rewards').select('id,promotion_id,restaurant_id,menu_item_id,custom_name,reward_type,reward_value,daily_limit,weight').eq('promotion_id', id).order('created_at');
  ...
  function addItem(item: any) { if (!promotion || !restaurant || rewards.length >= MAX || rewards.some((r) => r.menu_item_id === item.id)) return; setSaved(false); setRewards((a) => [...a, { temp_id: tid(), promotion_id: promotion.id, restaurant_id: restaurant.id, menu_item_id: item.id, custom_name: null, label: item.name, reward_type: 'discount', reward_value: 10, daily_limit: 10, weight_label: 'Normal', weight: 30 }]); }
  function addCustom() { if (!promotion || !restaurant || rewards.length >= MAX) return; setSaved(false); setRewards((a) => [...a, { temp_id: tid(), promotion_id: promotion.id, restaurant_id: restaurant.id, menu_item_id: null, custom_name: 'Custom Reward', label: 'Custom Reward', reward_type: 'custom', reward_value: null, daily_limit: 10, weight_label: 'Normal', weight: 30 }]); }
  ...
  return <main className="min-h-screen bg-[#FFF8F0] px-4 py-6 text-[#1F1F1F] sm:px-6"><section className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#FF6B00] to-[#E63939] p-5 text-white shadow-2xl shadow-orange-200 sm:p-8"><div className="grid gap-8 lg:grid-cols-[1fr_.9fr] lg:items-center"><div><p className="text-sm font-black uppercase tracking-[0.18em] text-white/75">SpinBite builder</p><h1 className="mt-3 text-4xl font-black leading-tight sm:text-6xl">{promotion.name}</h1><div className="mt-5 rounded-3xl bg-white/15 p-4 backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.16em] text-white/70">Restaurant location</p><p className="mt-1 text-2xl font-black">{restaurant.name}</p><p className="mt-1 text-sm font-bold text-white/85">{address}</p><div className="mt-3 flex flex-col gap-2 rounded-2xl bg-white/15 p-3 sm:flex-row sm:items-center sm:justify-between"><p className="break-all text-xs font-black text-white/85">{full}</p><button onClick={copyLink} className="shrink-0 rounded-full bg-white px-4 py-2 text-xs font-black text-[#FF6B00]">{copied ? 'Copied!' : 'Copy'}</button></div></div><div className="mt-4 flex flex-wrap gap-3"><span className="rounded-full bg-white px-4 py-2 text-sm font-black text-[#1F1F1F]">Status: {promotion.status}</span><button onClick={() => document.getElementById('test-mode')?.scrollIntoView({ behavior: 'smooth' })} className="rounded-full bg-[#1F1F1F] px-4 py-2 text-sm font-black text-white shadow-lg">Test</button></div></div><div className="rounded-[2rem] bg-white/90 p-4 text-[#1F1F1F] shadow-2xl ring-1 ring-white/50"><SpinWheelPreview rewards={rewards} rotation={rotation} spinning={spinning} /></div></div></section>
