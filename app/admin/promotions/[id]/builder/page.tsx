...
type BuilderStatus = 'draft' | 'pending' | 'active' | 'ended';
...
// promotions.game_type uses 'wheel' as a DB legacy alias for 'spin_wheel'. Normalise before comparison.
...
function getBuilderStatus(promotion: Promotion | null, startInput: string, endInput: string): BuilderStatus {
  if (!promotion || promotion.status === 'draft') return 'draft';
  const now = new Date();
  const start = startInput ? new Date(startInput) : promotion.starts_at ? new Date(promotion.starts_at) : null;
  const end = endInput ? new Date(endInput) : promotion.ends_at ? new Date(promotion.ends_at) : null;

  if (promotion.status === 'ended') return 'ended';
  if (end && end <= now) return 'ended';
  if (promotion.status === 'active' && start && start > now) return 'pending';
  if (promotion.status === 'active') return 'active';
  return 'draft';
}

function statusBadgeClass(status: BuilderStatus) {
...
export default function PromotionBuilderPage() {
  ...
  const [expiryPreset, setExpiryPreset] = useState<string>('20');
  ...
  const [gamePool, setGamePool] = useState<string[]>([]);
  const [availableGames, setAvailableGames] = useState<ActiveGame[]>([]);
  const [primaryGameType, setPrimaryGameType] = useState<string>('spin_wheel');
  ...
  const builderStatus = useMemo(() => getBuilderStatus(promotion, startsAt, endsAt), [promotion, startsAt, endsAt]);
  const isLive = builderStatus === 'active';
  const isPending = builderStatus === 'pending';
  const isEnded = builderStatus === 'ended';
  ...
  useEffect(() => {
    async function load() {
      ...
      const loadedPromotion = promotionResult.data as Promotion;
      setPromotion(loadedPromotion);
      setDailyLimit(loadedPromotion.daily_redeem_limit || 50);
      setMaxSpins(loadedPromotion.max_spins || 3);
      const loadedMinutes = loadedPromotion.coupon_expiry_minutes || 20;
      setCouponExpiryMinutes(loadedMinutes);
      const matchedPreset = EXPIRY_PRESETS.find((p) => p.minutes === loadedMinutes);
      setExpiryPreset(matchedPreset ? String(matchedPreset.minutes) : 'custom');
      if (!matchedPreset) {
        if (loadedMinutes % 1440 === 0) { setCustomExpiryValue(loadedMinutes / 1440); setCustomExpiryUnit('days'); }
        else if (loadedMinutes % 60 === 0) { setCustomExpiryValue(loadedMinutes / 60); setCustomExpiryUnit('hours'); }
        else { setCustomExpiryValue(loadedMinutes); setCustomExpiryUnit('minutes'); }
      }
      setStopOnWin(loadedPromotion.stop_on_win ?? true);
      setStartsAt(toLocalDateTimeInput(loadedPromotion.starts_at));
      setEndsAt(toLocalDateTimeInput(loadedPromotion.ends_at));
      ...
      // Load game assignments with is_primary to identify the primary game.
      const { data: assignmentData } = await supabase
        .from('promotion_game_assignments')
        .select('game_type, is_primary')
        .eq('promotion_id', id)
        .eq('enabled', true);

      // Primary game: from is_primary=true assignment, falling back to promotions.game_type (legacy).
      const primaryAssignment = (assignmentData ?? []).find((a: any) => a.is_primary);
      const resolvedPrimary = normalizePrimary(primaryAssignment?.game_type ?? loadedPromotion.game_type);
      setPrimaryGameType(resolvedPrimary);

      // Additional games: enabled assignments that are not the primary.
      setGamePool((assignmentData ?? []).filter((a: any) => !a.is_primary).map((a: any) => a.game_type));
  ...
  useEffect(() => {
    async function loadItems() {
      ...
      const result = await supabase
        .from('menu_items')
        .select('id,category_id,restaurant_id,name,price,image_url')
        .eq('category_id', menuId)
        .order('name');
      setMenuItems(result.data || []);
      if ((result.data || []).length === 0) {
        setMenuNotice('This category has no items yet. Add items in the Menu builder before assigning rewards.');
      }
    }
    loadItems();
  }, [menuId]);
  ...
  const errors = useMemo(() => {
    ...
    rewards.forEach((reward, index) => {
      ...
      if (reward.reward_type === 'discount' && (!reward.reward_value || reward.reward_value < 1 || reward.reward_value > 100)) messages.push(`Reward ${index + 1} needs a valid discount.`);
  ...
  async function savePromotion(nextStatus?: 'draft' | 'active') {
    ...
    setError('');
    const supabase = createClient();
    const statusToSave = nextStatus || promotion.status || 'draft';

    const promotionUpdate = await supabase.from('promotions').update({
    ...
    if (promotionUpdate.error) {
      setError(promotionUpdate.error.message);
      setSaving(false);
      return false;
    }

    const deleteRewards = await supabase.from('promotion_rewards').delete().eq('promotion_id', promotion.id);
    if (deleteRewards.error) {
      setError(deleteRewards.error.message);
      setSaving(false);
      return false;
    }

    if (rewards.length) {
      const insertRewards = await supabase.from('promotion_rewards').insert(rewards.map((reward) => ({
      ...
      if (insertRewards.error) {
        setError(insertRewards.error.message);
        setSaving(false);
        return false;
      }
    }
  ...
  async function launch() {
    if (errors.length || !promotion || !restaurant) return;
    setLaunching(true);
    setSaved(false);
    setLaunchSuccess(false);
    setError('');
    const savedLaunch = await savePromotion('active');
    if (!savedLaunch) {
      setLaunching(false);
      return;
    }
    await createClient().from('restaurants').update({ current_promotion_id: promotion.id }).eq('id', restaurant.id);
  ...
  function testSpin() {
    ...
    setTimeout(() => {
      setSpinning(false);
      setResult(rewards[index]?.label || 'Reward');
      confetti({ particleCount: 160, spread: 95, origin: { y: 0.62 } });
    }, 2900);
  }

  async function copyLink() {
    if (!restaurant || !promotion) return;
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#FFF8F0] px-3 py-6 text-[#1F1F1F] sm:px-6">
      <section className="mx-auto max-w-6xl space-y-6">
        <div className="flex items-center justify-between gap-4">
          <a href={`/admin/promotions?mode=manage&slug=${restaurant.slug}`} className="rounded-full bg-white px-5 py-3 text-sm font-black text-[#FF6B00] shadow">Back</a>
          <button onClick={copyLink} className="rounded-full bg-white px-5 py-3 text-sm font-black text-[#FF6B00] shadow">{copied ? 'Copied!' : 'Copy Link'}</button>
        </div>

        <section className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#FF6B00] to-[#E63939] p-4 text-white shadow-2xl shadow-orange-200 sm:p-8">
          <div className="grid min-w-0 gap-6 lg:grid-cols-[1fr_.82fr] lg:items-center">
            <div className="min-w-0">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2"><span className="text-2xl">🎯</span><span className="text-2xl font-black leading-none">SpinBite</span></div>
                  <p className="mt-2 text-xs font-black uppercase tracking-[0.18em] text-white/75">Promotion Builder</p>
                </div>
                <span className={`shrink-0 rounded-full px-3 py-2 text-xs font-black uppercase shadow ${statusBadgeClass(builderStatus)}`}>{builderStatus}</span>
              </div>
              <h1 className="mt-5 break-words text-4xl font-black leading-tight sm:text-6xl">{promotion.name}</h1>
              <div className="mt-5 rounded-3xl bg-white/15 p-4 backdrop-blur">
                ...
                <p className="mt-1 break-words text-2xl font-black">{restaurant.name}</p>
                <p className="mt-1 text-sm font-bold text-white/85">{address}</p>
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  <p className="rounded-2xl bg-white/15 p-3 text-xs font-black text-white/85">Start: {formatDisplayDate(fromLocalDateTimeInput(startsAt))}</p>
                  <p className="rounded-2xl bg-white/15 p-3 text-xs font-black text-white/85">End: {formatDisplayDate(fromLocalDateTimeInput(endsAt))}</p>
                </div>
              </div>
            </div>
        ...
        {saved && <div className="rounded-3xl bg-green-50 p-4 text-sm font-black text-green-700">Saved.</div>}
        ...
        <div className="rounded-[2rem] bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 1: Select Category</p><select value={menuId} onChange={(event) => setMenuId(event.target.value)} className="mt-3 w-full rounded-2xl border border-stone-200 px-4 py-4 font-black outline-none focus:border-[#FF6B00]"><option value="">Select a category</option>{menus.map((menu) => <option key={menu.id} value={menu.id}>{menu.name}{typeof menu.item_count === 'number' ? ` (${menu.item_count} items)` : ''}</option>)}</select>{menuNotice && <p className="mt-3 rounded-2xl bg-green-50 p-3 text-sm font-black text-green-700">{menuNotice}</p>}</div>

        <div className="rounded-[2rem] bg-white p-5 shadow-xl"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-black uppercase text-[#FF6B00]">Step 2: Add Rewards</p><p className="mt-2 text-sm font-bold text-stone-600">Choose 6–10 menu rewards. Add custom rewards only when needed.</p></div><button onClick={addCustom} disabled={rewards.length >= MAX} className="rounded-2xl bg-[#1F1F1F] px-4 py-3 text-sm font-black text-white disabled:bg-stone-300">Custom</button></div><div className="mt-4 rounded-2xl bg-orange-50 p-3 text-sm font-black text-[#FF6B00]">{rewardStatus}</div><div className="mt-4 grid gap-3 md:grid-cols-2">{menuItems.map((item) => { const added = rewards.some((reward) => reward.menu_item_id === item.id); return <div key={item.id} className="flex items-center justify-between gap-4 rounded-2xl border border-stone-100 p-4"><div className="flex min-w-0 items-center gap-3"><div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-stone-100">{item.image_url ? <img src={item.image_url} alt={item.name} className="h-full w-full object-cover" loading="lazy" /> : <div className="flex h-full w-full items-center justify-center text-xl">🍽️</div>}</div><div className="min-w-0"><p className="truncate text-lg font-black">{item.name}</p><p className="text-sm font-bold text-stone-500">{money(item.price)}</p></div></div><button onClick={() => addItem(item)} disabled={added || rewards.length >= MAX} className="shrink-0 rounded-full bg-[#FF6B00] px-4 py-2 text-sm font-black text-white disabled:bg-stone-300">{added ? 'Added' : 'Add'}</button></div>; })}{!menuItems.length && <p className="rounded-2xl bg-stone-50 p-4 text-sm font-bold text-stone-500 md:col-span-2">No items found in this category yet. Add items using the Menu builder.</p>}</div></div>
        ...
        <div className="rounded-[2rem] bg-white p-5 shadow-xl">
          ...
          {/* Primary Experience — from is_primary=true assignment (DB-authoritative). */}
          {(() => {
            const primaryMeta = getGameMeta(primaryGameType);
            return (
            ...
            return (
              <>
                ...
                <div className="mt-2 grid gap-3 sm:grid-cols-2">
                  ...
                  {additionalAvailableGames.length === 0 && <p className="rounded-2xl bg-stone-50 p-4 text-sm font-bold text-stone-400 sm:col-span-2">No additional active games available.</p>}
                </div>
              </>
            );
          })()}
