...
type GameType = 'wheel' | 'mystery_box' | 'scratch_card';
...
function MiniMysteryBox() {
  return <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-3xl bg-white text-4xl shadow-inner">🎁</div>;
}

function MiniScratchCard() {
  return <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-3xl bg-gradient-to-br from-orange-400 via-yellow-300 to-red-500 text-4xl shadow-inner">🪙</div>;
}
...
export default function PromotionsPage() {
  ...
  return <main className="min-h-screen bg-[#FFF8F0] px-4 py-6 text-[#1F1F1F]"><section className="mx-auto max-w-5xl">
    ...
    {mode === 'create' && <><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">{copy.name_promotion_label}</p><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Halloween, Lunch Rush, Weekend Spin..." className="mt-3 w-full rounded-2xl border border-stone-200 px-4 py-4 font-semibold outline-none focus:border-[#FF6B00]" /></div><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">{copy.select_game_label}</p><GameCard selected={gameType === 'wheel'} onClick={() => setGameType('wheel')}><div className="flex items-start gap-4"><MiniPrizeWheel /><div><p className="text-2xl font-black">Spin Wheel</p><p className="mt-1 text-sm font-bold text-stone-600">Customers scan a QR code, spin a branded prize wheel, and win configured rewards.</p><p className="mt-2 text-xs font-black uppercase text-green-700">Available now</p></div></div></GameCard><GameCard selected={gameType === 'mystery_box'} onClick={() => setGameType('mystery_box')}><div className="flex items-start gap-4"><MiniMysteryBox /><div><p className="text-2xl font-black">Mystery Box Reveal</p><p className="mt-1 text-sm font-bold text-stone-600">Customers tap one of 3 mystery boxes and reveal a surprise coupon with stars and confetti.</p><p className="mt-2 text-xs font-black uppercase text-green-700">Available now</p></div></div></GameCard><GameCard selected={gameType === 'scratch_card'} onClick={() => setGameType('scratch_card')}><div className="flex items-start gap-4"><MiniScratchCard /><div><p className="text-2xl font-black">Scratch Card</p><p className="mt-1 text-sm font-bold text-stone-600">Customers tap a digital scratch card to reveal a surprise reward using the same coupon engine.</p><p className="mt-2 text-xs font-black uppercase text-green-700">Available now</p></div></div></GameCard></div><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 4: {copy.create_button_label}</p><button onClick={addPromotion} disabled={!canCreate} className="mt-3 w-full rounded-3xl bg-green-600 px-5 py-5 text-xl font-black text-white shadow-xl disabled:bg-stone-400">{saving ? 'Creating...' : copy.create_button_label}</button></div></>}
