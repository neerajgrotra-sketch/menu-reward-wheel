...
function restaurantAddress(restaurant?: Restaurant | null) { return [restaurant?.address_line1, restaurant?.city].filter(Boolean).join(', ') || 'Address not added'; }
...
function formatDateTime(value?: string | null) { return value ? new Intl.DateTimeFormat('en-CA', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(value)) : 'No end date set'; }
...
export default function PromotionsPage() {
  ...
  async function endPromotion(event: React.MouseEvent, promotion: Promotion) {
    event.preventDefault(); event.stopPropagation();
    const status = effectiveStatus(promotion);
    if (status !== 'active') return;

    const confirmed = window.confirm(`End ${promotion.name} now? Customers will no longer be able to play this promotion.`);
    if (!confirmed) return;

    setEndingId(promotion.id);
    setError('');
    const endedAt = new Date().toISOString();
    const updateResult = await supabase.from('promotions').update({ ends_at: endedAt }).eq('id', promotion.id);
    ...
    setPromotions((current) => current.map((item) => item.id === promotion.id ? { ...item, ends_at: endedAt } : item));
    setEndingId(null);
  ...
  return (
    <main className="min-h-screen bg-[#FFF8F0] px-4 py-6 text-[#1F1F1F]"><section className="mx-auto max-w-5xl">
      ...
      <div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">{mode === 'create' ? 'Step 1: Select Restaurant Location' : 'Restaurant Location'}</p><select value={selectedRestaurantId} onChange={(event) => setSelectedRestaurantId(event.target.value)} className="mt-3 w-full rounded-2xl border border-stone-200 bg-white px-4 py-4 font-black outline-none focus:border-[#FF6B00]"><option value="">Select restaurant/location...</option>{restaurants.map((restaurant) => <option key={restaurant.id} value={restaurant.id}>{locationLabel(restaurant)}</option>)}</select>{selectedRestaurant && <div className="mt-4 rounded-2xl bg-orange-50 p-4"><p className="text-xl font-black">{selectedRestaurant.name}</p><p className="mt-1 text-sm font-bold text-stone-600">{restaurantAddress(selectedRestaurant)}</p><p className="mt-1 text-xs font-bold text-stone-500">/{selectedRestaurant.slug}</p></div>}</div>
      {mode === 'create' && <><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 2: Name Promotion</p><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Halloween, Lunch Rush, Weekend Spin..." className="mt-3 w-full rounded-2xl border border-stone-200 px-4 py-4 font-semibold outline-none focus:border-[#FF6B00]" /></div><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 3: Select Game Type</p><button onClick={() => setSelectedGame('wheel')} className={`mt-3 w-full rounded-3xl border-2 p-5 text-left transition ${selectedGame === 'wheel' ? 'border-green-600 bg-green-50' : 'border-stone-200 bg-white'}`}><div className="flex items-start gap-4"><MiniPrizeWheel /><div><p className="text-2xl font-black">Spin Wheel</p><p className="mt-1 text-sm font-bold text-stone-600">Customers scan a QR code, spin a branded prize wheel, and win configured rewards.</p><p className="mt-2 text-xs font-black uppercase text-green-700">Available now</p></div></div></button></div><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 4: Create Promotion</p><button onClick={addPromotion} disabled={!canCreate} className="mt-3 w-full rounded-3xl bg-green-600 px-5 py-5 text-xl font-black text-white shadow-xl disabled:bg-stone-400">{saving ? 'Creating...' : 'Create Promotion'}</button></div></>}
      {error && <p className="mt-5 rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-700">{error}</p>}
      <div className="mt-5 space-y-4"><div className="rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">{mode === 'create' ? 'Drafts in progress' : 'Managed promotions'}</p><p className="mt-2 text-sm font-bold text-stone-500">{mode === 'create' ? 'Drafts appear here while they are still being built. Published campaigns move to Manage Promotions.' : 'Active and ended campaigns stay here for operations, reporting, and relaunching.'}</p></div>{selectedRestaurant && listToShow.length === 0 && <div className="rounded-3xl bg-white p-6 shadow-xl"><p className="text-2xl font-black">{mode === 'create' ? 'No drafts in progress' : 'No managed promotions yet'}</p><p className="mt-2 text-sm font-semibold text-stone-600">{mode === 'create' ? 'Create a new draft above.' : 'Launch a draft first, then it will appear here.'}</p></div>}{selectedRestaurant && listToShow.map((promotion) => { const status = effectiveStatus(promotion); const metric = counts[promotion.id] || { issued: 0, redeemed: 0 }; const rate = metric.issued ? Math.round((metric.redeemed / metric.issued) * 100) : 0; return <div key={promotion.id} className="rounded-3xl bg-white p-5 shadow-xl"><div className="flex items-start justify-between gap-4"><div><h3 className="text-3xl font-black">{promotion.name}</h3><p className={`mt-2 inline-block rounded-full px-3 py-1 text-xs font-black uppercase ${status === 'active' ? 'bg-green-50 text-green-700' : status === 'ended' ? 'bg-stone-100 text-stone-600' : 'bg-orange-50 text-[#FF6B00]'}`}>{status}</p></div><a href={`/admin/promotions/${promotion.id}/builder`} className="rounded-full bg-orange-50 px-4 py-2 text-center text-sm font-black text-[#FF6B00]">{status === 'draft' ? 'Build' : 'Edit'}</a></div>{mode === 'manage' && <div className="mt-4 rounded-2xl bg-orange-50 p-4"><p className="text-xs font-black uppercase tracking-wide text-[#FF6B00]">Restaurant Location</p><p className="mt-1 text-xl font-black">{selectedRestaurant.name}</p><p className="mt-1 text-sm font-bold text-stone-600">{restaurantAddress(selectedRestaurant)}</p><p className="mt-3 text-xs font-black uppercase tracking-wide text-stone-500">Promotion Expiry</p><p className="mt-1 text-sm font-black text-stone-800">{formatDateTime(promotion.ends_at)}</p></div>}{mode === 'manage' && <div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-2xl bg-stone-50 p-3"><p className="text-xl font-black">{metric.issued}</p><p className="text-xs font-bold text-stone-500">Issued</p></div><div className="rounded-2xl bg-stone-50 p-3"><p className="text-xl font-black">{metric.redeemed}</p><p className="text-xs font-bold text-stone-500">Redeemed</p></div><div className="rounded-2xl bg-stone-50 p-3"><p className="text-xl font-black">{rate}%</p><p className="text-xs font-bold text-stone-500">Rate</p></div></div>}<p className="mt-4 break-all text-sm font-black text-[#FF6B00]">/play/{selectedRestaurant.slug}/{promotion.slug}</p><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={(event) => copyPlayLink(event, promotion)} className="rounded-2xl bg-[#1F1F1F] px-4 py-3 text-sm font-black text-white">{copiedId === promotion.id ? 'Copied!' : 'Copy Link'}</button>{status === 'draft' ? <button onClick={(event) => deletePromotion(event, promotion)} disabled={deletingId === promotion.id} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">{deletingId === promotion.id ? 'Deleting...' : 'Delete Draft'}</button> : status === 'active' ? <button onClick={(event) => endPromotion(event, promotion)} disabled={endingId === promotion.id} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">{endingId === promotion.id ? 'Ending...' : 'End Promotion'}</button> : <span className="rounded-2xl bg-stone-100 px-4 py-3 text-center text-sm font-black text-stone-500">Promotion Ended</span>}</div></div>; })}</div>
    </section></main>
  );
}
