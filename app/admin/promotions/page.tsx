...
export default function PromotionsPage() {
  ...
  return <main className="min-h-screen bg-[#FFF8F0] px-4 py-6 text-[#1F1F1F]"><section className="mx-auto max-w-5xl">
    ...
    {mode === 'create' && <><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 2: Name Promotion</p><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Halloween, Lunch Rush, Weekend Spin..." className="mt-3 w-full rounded-2xl border border-stone-200 px-4 py-4 font-semibold outline-none focus:border-[#FF6B00]" /></div><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 3: Select Game Type</p><button onClick={() => undefined} className="mt-3 w-full rounded-3xl border-2 border-green-600 bg-green-50 p-5 text-left"><div className="flex items-start gap-4"><MiniPrizeWheel /><div><p className="text-2xl font-black">Spin Wheel</p><p className="mt-1 text-sm font-bold text-stone-600">Customers scan a QR code, spin a branded prize wheel, and win configured rewards.</p><p className="mt-2 text-xs font-black uppercase text-green-700">Available now</p></div></div></button></div><div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Step 4: Create Promotion</p><button onClick={addPromotion} disabled={!canCreate} className="mt-3 w-full rounded-3xl bg-green-600 px-5 py-5 text-xl font-black text-white shadow-xl disabled:bg-stone-400">{saving ? 'Creating...' : 'Create Promotion'}</button></div></>}
    {error && <p className="mt-5 rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-700">{error}</p>}
    <div className="mt-5 space-y-4"><div className="rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">{mode === 'create' ? 'Drafts in progress' : 'Managed promotions'}</p><p className="mt-2 text-sm font-bold text-stone-500">{mode === 'create' ? 'Drafts appear here while they are still being built. Published campaigns move to Manage Promotions.' : 'Default view shows active promotions. Filter to drafts or ended campaigns when needed.'}</p>{mode === 'manage' && <div className="mt-4 grid grid-cols-4 gap-2 rounded-2xl bg-stone-50 p-2">{(['active','draft','ended','all'] as Filter[]).map((f) => <button key={f} onClick={() => setFilter(f)} className={`rounded-xl px-2 py-3 text-xs font-black ${filter === f ? 'bg-[#1F1F1F] text-white shadow' : 'bg-white text-stone-600'}`}>{f === 'all' ? 'All' : f[0].toUpperCase() + f.slice(1)}<br />{statusCounts[f]}</button>)}</div>}</div>{selectedRestaurant && visiblePromotions.length === 0 && <div className="rounded-3xl bg-white p-6 shadow-xl"><p className="text-2xl font-black">{mode === 'create' ? 'No drafts in progress' : `No ${filter} promotions`}</p><p className="mt-2 text-sm font-semibold text-stone-600">{mode === 'create' ? 'Create a new draft above.' : 'Switch filters to view other statuses.'}</p></div>}{selectedRestaurant && visiblePromotions.map((p) => { const s = statusOf(p); const metric = counts[p.id] || { issued: 0, redeemed: 0 }; const rate = metric.issued ? Math.round((metric.redeemed / metric.issued) * 100) : 0; return <div key={p.id} className="rounded-3xl bg-white p-5 shadow-xl"><div className="flex items-start justify-between gap-4"><div><h3 className="text-3xl font-black">{p.name}</h3><p className={`mt-2 inline-block rounded-full px-3 py-1 text-xs font-black uppercase ${s === 'active' ? 'bg-green-50 text-green-700' : s === 'ended' ? 'bg-stone-100 text-stone-600' : 'bg-orange-50 text-[#FF6B00]'}`}>{s}</p></div><a href={`/admin/promotions/${p.id}/builder`} className="rounded-full bg-orange-50 px-4 py-2 text-center text-sm font-black text-[#FF6B00]">{s === 'draft' ? 'Build' : 'Edit'}</a></div>{mode === 'manage' && <><div className="mt-4 rounded-2xl bg-orange-50 p-4"><p className="text-xs font-black uppercase tracking-wide text-[#FF6B00]">Restaurant Location</p><p className="mt-1 text-xl font-black">{selectedRestaurant.name}</p><p className="mt-1 text-sm font-bold text-stone-600">{address(selectedRestaurant)}</p><p className="mt-3 text-xs font-black uppercase tracking-wide text-stone-500">Promotion Expiry</p><p className="mt-1 text-sm font-black text-stone-800">{fmt(p.ends_at)}</p></div><div className="mt-4 grid grid-cols-3 gap-2 text-center"><div className="rounded-2xl bg-stone-50 p-3"><p className="text-xl font-black">{metric.issued}</p><p className="text-xs font-bold text-stone-500">Issued</p></div><div className="rounded-2xl bg-stone-50 p-3"><p className="text-xl font-black">{metric.redeemed}</p><p className="text-xs font-bold text-stone-500">Redeemed</p></div><div className="rounded-2xl bg-stone-50 p-3"><p className="text-xl font-black">{rate}%</p><p className="text-xs font-bold text-stone-500">Rate</p></div></div></>}<p className="mt-4 break-all text-sm font-black text-[#FF6B00]">/play/{selectedRestaurant.slug}/{p.slug}</p><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={(event) => copyPlayLink(event, p)} className="rounded-2xl bg-[#1F1F1F] px-4 py-3 text-sm font-black text-white">{copiedId === p.id ? 'Copied!' : 'Copy Link'}</button><a href={`/admin/promotions/${p.id}/print`} target="_blank" className="rounded-2xl bg-green-600 px-4 py-3 text-center text-sm font-black text-white">Print Kit</a>{s === 'draft' ? <button onClick={(event) => deletePromotion(event, p)} disabled={deletingId === p.id} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">{deletingId === p.id ? 'Deleting...' : 'Delete Draft'}</button> : s === 'active' ? <button onClick={(event) => endPromotion(event, p)} disabled={endingId === p.id} className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">{endingId === p.id ? 'Ending...' : 'End Promotion'}</button> : <span className="rounded-2xl bg-stone-100 px-4 py-3 text-center text-sm font-black text-stone-500">Promotion Ended</span>}</div></div>; })}</div>
  </section></main>;
}
