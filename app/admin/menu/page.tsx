...
export default function MenuPage() {
  ...
  async function loadMenus(restaurantId: string) {
    const { data: menusData, error: menusError } = await supabase.from('menus').select('id,name,menu_type').eq('restaurant_id', restaurantId);
  ...
  async function loadItems(menuId: string) {
    const { data, error: itemsError } = await supabase.from('menu_items').select('id,name,price').eq('menu_id', menuId);
  ...
  useEffect(() => {
    async function init() {
      ...
      let query = supabase.from('restaurants').select('id,name,slug').eq('owner_id', user.id).limit(1);
      if (requestedSlug) query = query.eq('slug', requestedSlug);
      const { data: restaurantData, error: restaurantError } = await query.single();
      if (restaurantError || !restaurantData) { window.location.href = '/admin/restaurants'; return; }
  ...
  async function deleteMenu(menu: Menu) {
    ...
    const ok = window.confirm(`Delete ${menu.name} menu?`);
    if (!ok) return;
    setError('');
    await supabase.from('menu_items').delete().eq('menu_id', menu.id);
    const { error: menuError } = await supabase.from('menus').delete().eq('id', menu.id);
    ...
    <main className="min-h-screen bg-[#FFF8F0] px-4 py-6 text-[#1F1F1F]">
      <section className="mx-auto max-w-5xl">
        ...
        <div className="mt-6 rounded-[2rem] bg-gradient-to-br from-[#FF6B00] to-[#E63939] p-6 text-white shadow-2xl shadow-orange-200">
          ...
          <p className="mt-3 text-sm font-semibold text-white/85">Add items with optional CAD prices. Promotions will later pull reward items directly from here.</p>
          {restaurant && <p className="mt-4 rounded-2xl bg-white/15 p-3 text-sm font-black">Restaurant: {restaurant.name}</p>}
        </div>
        <div className="mt-5 rounded-3xl bg-white p-5 shadow-xl"><p className="text-sm font-black uppercase text-[#FF6B00]">Create Menu</p><div className="mt-3 flex gap-2"><input value={newMenu} onChange={(e) => setNewMenu(e.target.value)} placeholder="Breakfast, Lunch, Dinner..." className="min-w-0 flex-1 rounded-2xl border border-stone-200 px-4 py-3 font-semibold outline-none focus:border-[#FF6B00]" /><button onClick={addMenu} className="rounded-2xl bg-green-600 px-5 py-3 text-xl font-black text-white">+</button></div></div>
        ...
        <div className="mt-5 space-y-4">
          {menus.length === 0 && <div className="rounded-3xl bg-white p-6 shadow-xl"><p className="text-2xl font-black">No menus yet</p><p className="mt-2 text-sm font-semibold text-stone-600">Create your first menu, then add items.</p></div>}
          {menus.map((menu) => {
