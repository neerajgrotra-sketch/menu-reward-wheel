...
type Promotion = { id: string; name: string; slug: string; status: string; coupon_expiry_minutes?: number | null; starts_at?: string | null; ends_at?: string | null; max_spins?: number | null; stop_on_win?: boolean | null };
type PromotionReward = { id: string; menu_item_id: string | null; custom_name: string | null; reward_type: 'free' | 'discount' | 'custom'; reward_value: number | null; daily_limit: number | null; weight: number | null };
...
export default function PromotionPlayPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const promotionResult = await supabase.from('promotions').select('id,name,slug,status,coupon_expiry_minutes,starts_at,ends_at,max_spins,stop_on_win').eq('restaurant_id', currentRestaurant.id).eq('slug', promotionSlug).single();
      ...
      const rewardsResult = await supabase.from('promotion_rewards').select('id,menu_item_id,custom_name,reward_type,reward_value,daily_limit,weight').eq('promotion_id', currentPromotion.id).order('created_at', { ascending: true });
  ...
  function spin() {
    ...
    setTimeout(async () => {
      ...
      if (!insertResult.error && insertResult.data?.id) redemptionId = insertResult.data.id;
          ...
          <section className="mt-6 rounded-3xl bg-white p-5 shadow-xl">
            ...
            <div className="mt-4 space-y-4">
              {wonCoupons.map((item, index) => {
                ...
                return (
                  <button key={item.id} onClick={() => { setActiveCouponId(item.id); setShowReveal(true); }} className="relative w-full rounded-2xl border border-stone-200 bg-stone-50 p-4 text-left shadow-sm">
                    ...
                    <p className={expired ? 'mt-1 text-sm font-black text-red-600' : 'mt-1 text-sm font-bold text-green-700'}>{expired ? 'Expired' : `Expires in ${formatRemaining(expiresAt - now)}`}</p>
                  </button>
                );
              })}
            </div>
      ...
      {showReveal && activeCoupon && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/40 px-3 pb-3 backdrop-blur-sm">
          <section className="mx-auto w-full max-w-md rounded-[2rem] bg-white p-5 text-center shadow-2xl">
            ...
            <p className={activeExpired ? 'mt-4 text-lg font-black text-red-600' : 'mt-4 text-lg font-bold text-red-600'}>{activeExpired ? 'Coupon expired' : `Expires in ${formatRemaining((activeExpiresAt || 0) - now)}`}</p>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <button onClick={spin} disabled={!canSpin} className="rounded-2xl bg-green-600 px-5 py-4 text-sm font-black text-white disabled:bg-stone-300">{spinsRemaining > 0 ? 'Spin Again' : 'No Spins Left'}</button>
            </div>
