...
import { reconstructSession } from '@/lib/session-intelligence';
...
export async function GET(
  ...
  try {
    ...
    // Fetch all orders placed during this session with their line items
    const { data: orders, error: ordersError } = await supabase
      .from('orders')
      .select(
        'id,order_number,status,subtotal,created_at,order_items(id,menu_item_id,name_snapshot,quantity,price_snapshot,effective_price_snapshot,line_total,special_instructions)',
      )
